...
output "bastion_floating_ip" {
  description = "Bastion 的 Floating IP (kolla-ansible 部署入口)"
  value       = zillaforge_floating_ip.bastion.ip_address
}

output "controller_floating_ip" {
  description = "OpenStack Controller 的 Floating IP（僅在 enable_controller_fip = true 時才會配置）"
...
output "default_network_ips" {
  ...
  value = merge(
    ...
    { (zillaforge_server.controller.name) = zillaforge_server.controller.network_attachment[0].ip_address },
    { for s in zillaforge_server.workers : s.name => s.network_attachment[0].ip_address }
  )
}
